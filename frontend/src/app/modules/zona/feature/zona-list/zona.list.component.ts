...
export class ZonaPageListComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.searchZonaChanged.pipe(
            ...
            debounceTime(800),
            tap(searchValue => {
                this.zonaStore.changeQueryInZona(searchValue);
            })
        ).subscribe();
    }
    ...
    public changePagination(event: any) {
        this.zonaStore.changePageInTipoDocumento(event);
    }

    public loadAllStatesFixedAsset() {
        this.zonaStore.loadAllZonaStore();
    }
