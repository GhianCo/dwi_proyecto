...
import { VendedorMapper } from './mappers/vendedor.mapper';

@Injectable({
    providedIn: 'root'
})

export class VendedorRemoteReq {

    private vendedorMapper = new VendedorMapper();
    ...
    requestSearchVendedorByCriteria(criteria): Observable<IResponse> {
        return this.http.post(this.REMOTE_API_URI + 'vendedor/searchByParams', criteria)
            .pipe(
                map((response: any) => {
                    if (response.data) {
                        response.data = this.vendedorMapper.transform(response.data);
                    }
                    return response;
                })
            );
